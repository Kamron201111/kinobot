...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    if not await require_subscription(user.id):
        ...
        await msg.answer(T.must_subscribe(), reply_markup=K.subscribe_kb(not_subbed))
        return

    is_admin = await db.is_admin(user.id)
    is_prem = await db.is_premium(user.id)
    await msg.answer(
        T.welcome(user.first_name or "do'stim", is_prem),
...
async def cb_back_main(cb: CallbackQuery, state: FSMContext):
    ...
    await safe_edit(cb, T.welcome(cb.from_user.first_name or "do'stim", is_prem),
...
async def cb_check_subs(cb: CallbackQuery, state: FSMContext):
    if await require_subscription(cb.from_user.id):
        ...
        await safe_edit(cb, T.still_not_subscribed(), K.subscribe_kb(not_subbed))
...
async def cb_watch(cb: CallbackQuery, state: FSMContext):
    ...
    if not await require_subscription(cb.from_user.id):
        not_subbed = await get_unsubscribed(cb.from_user.id)
        await safe_edit(cb, T.must_subscribe(), K.subscribe_kb(not_subbed))
        await cb.answer()
        return
    s = await db.all_settings()
    movies_link = s.get("movies_channel") or None
    await safe_edit(cb, T.watch_intro(), K.watch_menu(movies_link))
    await cb.answer()
...
async def cb_enter_code(cb: CallbackQuery, state: FSMContext):
    ...
    await safe_edit(cb, T.ask_code(), K.back_to_watch())
    await cb.answer()
...
async def on_code(msg: Message, state: FSMContext):
    ...
    if not movie:
        await msg.answer(T.code_not_found(code), reply_markup=K.back_to_watch())
        return
    await state.clear()
...
async def cb_main_channel(cb: CallbackQuery):
    ...
    await safe_edit(cb, T.main_channel_text(link), K.back_main())
    await cb.answer()
...
async def cb_buy_premium(cb: CallbackQuery, state: FSMContext):
    ...
    await safe_edit(cb, T.premium_offer(prices, stars), K.premium_plans(prices))
    await cb.answer()
...
async def cb_plan(cb: CallbackQuery):
    ...
    await safe_edit(
        cb,
        T.choose_method(label, prices[plan], stars[plan]),
...
async def cb_pay_card(cb: CallbackQuery):
    ...
    await safe_edit(
        cb,
        T.card_instructions(method, card, name, PLAN_LABELS[plan], prices[plan]),
...
async def cb_sent_receipt(cb: CallbackQuery, state: FSMContext):
    ...
    await safe_edit(cb, T.send_receipt(), K.back_main())
    await cb.answer()
...
async def on_receipt(msg: Message, state: FSMContext):
    ...
    await msg.answer(T.receipt_received(), reply_markup=K.back_main())
...
async def on_success_payment(msg: Message):
    ...
    if len(parts) >= 2 and parts[0] == "stars":
        ...
        if days:
            ...
            await msg.answer(T.premium_activated(until_str, days),
...
async def movie_file(msg: Message, state: FSMContext):
    ...
    announce = T.new_movie_for_premium(data["code"], data["title"], data["caption"])
    sent = 0
    for uid in prem_ids:
        try:
            if ftype == "document":
...
async def cb_pay_ok(cb: CallbackQuery):
    ...
    try:
        await bot.send_message(
            pay["user_id"],
            T.premium_activated(until_str, days),
            reply_markup=K.premium_success(),
        )
    except (TelegramForbiddenError, TelegramBadRequest):
        pass
...
async def cb_pay_no(cb: CallbackQuery):
    ...
    try:
        await bot.send_message(
            pay["user_id"],
            f"{E.e('cross')} <b>To'lov tasdiqlanmadi</b>\n\n"
            f"Chek noto'g'ri yoki to'lov topilmadi. Iltimos, admin bilan bog'laning.",
